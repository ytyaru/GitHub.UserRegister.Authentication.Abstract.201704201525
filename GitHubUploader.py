...
#        print(os.path.join(path_dir_db, 'GitHub.Repositories.{0}.sqlite3'.format(args.username)))
        ...
        creator = web.service.github.api.v3.AuthenticationsCreator.AuthenticationsCreator(db, args.username)
...
#        user = web.service.github.api.v3.CurrentUser.CurrentUser(db, args.username)
        ...
        authData.Load(db.account, args.username)
        client = web.service.github.api.v3.Client.Client(db, authentications, authData=authData, repo=repo)
        main = cui.uploader.Main.Main(db, client, authData, repo)
        main.Run()
